...
letter_AA = list("ARNDCQEGHILKMFPSTWYV-")
num_AA = [i for i in range(21)]
aa_N = {i: j for i, j in zip(num_AA,letter_AA)}
def Num2AA(x):
    y=[list(np.where(y in num_AA,list(aa_N[y]),list("-")))[0] for y in x]
    return("".join(y))
